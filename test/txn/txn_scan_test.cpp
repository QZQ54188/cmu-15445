...
TEST(TxnScanTest, ScanTest) {  // NOLINT
  ...
  WithTxn(txn2, QueryHideResult(*bustub, _var, _txn, query, IntResult{{2}, {6}})); // txn2 读取时间戳为2，能看到所有在ts=2及之前提交的事务
  fmt::println(stderr, "D: Verify txn3");
  WithTxn(txn3, QueryHideResult(*bustub, _var, _txn, query, IntResult{{2}, {3}, {5}})); // txn3 读取时间戳为3，能看到ts=3及之前提交的事务
  fmt::println(stderr, "E: Verify txn4");
  WithTxn(txn4, QueryHideResult(*bustub, _var, _txn, query, IntResult{{1}, {3}, {6}})); // txn4 读取时间戳为4，能看到ts=4及之前提交的事务
  fmt::println(stderr, "F: Verify txn5");
  WithTxn(txn5, QueryHideResult(*bustub, _var, _txn, query, IntResult{{2}, {3}, {6}})); // txn5 读取时间戳为5，能看到所有事务
}
